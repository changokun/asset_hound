...
class asset {
	...
	static function get_asset_by_keyword($keyword = '') {
		...
		//m::death(self::$assets_by_keyword, 'all theknown keywords for assets.');
	...
	private static function load_asset_keywords() {
		// todo - this data will probably be read from a db or ini file, and since it involves waking serialized assetobjects. we should cache it somewehere. memcache?
		// for now, create an array of asset objects
		self::$assets_by_keyword = array(
			'mBox' => array(
				new asset('http://assets.sarumino.com/lib/mBox/mBox.js'),
				new asset('/assets/lib/mBox/mBox_extension.js'), // todo this is sarumino only.
				new asset('http://assets.sarumino.com/lib/mBox/mBox.css')
			),
			'term_browse' => array(
				new asset('http://assets.sarumino.dev/xio/lib/term_browse/term_browse.js'),
				new asset('http://assets.sarumino.dev/xio/lib/term_browse/term_browse.css')
			),
			'jQuery_1.7' => array(
				new asset('https://ajax.googleapis.com/ajax/libs/jquery/1.7/jquery.min.js')
			),
			'skeleton_1.2' => array(
				new asset('http://assets.sarumino.com/lib/Skeleton/1.2/base.css', 'framework'),
				new asset('http://assets.sarumino.com/lib/Skeleton/1.2/layout.css', 'framework'),
				new asset('http://assets.sarumino.com/lib/Skeleton/1.2/skeleton.css', 'framework')
			)
		);
	}
